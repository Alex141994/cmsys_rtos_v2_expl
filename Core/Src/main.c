...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	for(int i= 6; i > 0; i--) {
		//HAL_Delay(400);
		HAL_GPIO_TogglePin(GPIOE, HL1_Pin);
	}
  /* USER CODE END 2 */
  ...
  /* USER CODE BEGIN RTOS_TIMERS */
  	if(myTimer01Handle != NULL) {
		osStatus_t status = osTimerStart(myTimer01Handle, 400);       // start timer
    if (status != osOK) {
	...
	if(myTimer02Handle != NULL) {
		osStatus_t status = osTimerStart(myTimer02Handle, 500);       // start timer
    if (status != osOK) {
      // Timer could not be started
			while(1) {
				HAL_Delay(300);
